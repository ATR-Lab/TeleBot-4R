...
private:
    ...
    void relay()
    {
        ...
        try
        {
            ...
            for (const auto &it : _motorGoalBuffer)
            {
                ...
                msg.motor_goal_position.push_back(it.second);
            }
    ...
    {
        size_t motorGoalsLength = msg.motor_goal_position.size();
        if (motorGoalsLength != msg.motor_id.size())
        {
            RCLCPP_ERROR(this->get_logger(), "Multiplexer refused relay! Unequal count of entries in motor goals and motor ids.");
            return;
        }
        for (size_t i = 0; i < motorGoalsLength; i++)
        {
            ...
            auto pair=std::make_pair(msg.motor_id[i],msg.motor_goal_position[i]);
            auto res=_motorGoalBuffer.insert(pair);
            if(!res.second){
                res.first->second=msg.motor_goal_position[i];
            }
        }
    }
    ...
    std::unordered_map<int16_t, float> _motorGoalBuffer;
